  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = bubblesRef.current.length - 1; i >= 0; i--) {
        ...
        ctx.fillStyle = `rgba(16,185,129,${(b.alpha * fade * 0.9).toFixed(3)})`; // emerald green
        ctx.fill();
      ...
      ctx.strokeStyle = `rgba(16,185,129,${(0.25 + volumeLevel * 0.2).toFixed(3)})`;
      ctx.lineWidth = 3;
      ctx.stroke();
      // inner core glow
      ctx.beginPath();
      ctx.arc(centerX, centerY, 11 * (0.9 + 0.15 * Math.cos(now / 180) * pulse), 0, Math.PI * 2);
      ctx.fillStyle = `rgba(16,185,129,${(0.95 - volumeLevel * 0.2).toFixed(3)})`;
      ctx.fill();
